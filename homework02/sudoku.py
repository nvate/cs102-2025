...
def get_block(grid: tp.List[tp.List[str]], pos: tp.Tuple[int, int]) -> tp.List[str]:
    ...
    start_row = (x // 3) * 3
    start_col = (y // 3) * 3
    block = [grid[start_row + i][start_col + j] for i in range(3) for j in range(3)]
...
def find_empty_positions(
    ...
    for row in grid:
        for i in row:
            if i == ".":
                return (grid.index(row), row.index(i))
...
def find_possible_values(grid: tp.List[tp.List[str]], pos: tp.Tuple[int, int]) -> tp.Set[str]:
    ...
    for j in grid:
        if j[pos[1]] in all_possible_values:
            all_possible_values.remove(j[pos[1]])

    for k in get_block(grid, pos):
        if k in all_possible_values:
            all_possible_values.remove(k)
...
def generate_sudoku(N: int) -> tp.List[tp.List[str]]:
    ...
    for block in range(3):
        ...
        for i in range(3):
            for j in range(3):
                grid[block * 3 + i][block * 3 + j] = numbers[idx]
                idx += 1
